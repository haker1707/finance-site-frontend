...
function schedule(p){if(p.schedule)return p.schedule;return split(p.amount,p.count).map((amount,i)=>{const m=addMonths(p.firstDue,i);return {date:date(+m.slice(0,4),+m.slice(5,7),p.dueDay||+p.firstDue.slice(8)),amount};});}
...
function ledger(records){const tx=records.filter(r=>r.kind==='entry').map(r=>({...r,derived:false}));for(const p of records.filter(r=>r.kind==='purchase'))for(const [i,x] of schedule(p).entries())tx.push({id:p.id+':'+i,kind:'entry',date:x.date,amount:x.amount,type:'despesa',categoryId:p.categoryId,description:`${p.name} · ${i+1}/${schedule(p).length}`,cardId:p.cardId,purchaseId:p.id,derived:true});return tx;}
...
module.exports={MONTHS,TYPES,money,integer,validDate,date,addMonths,split,firstDue,schedule,reserve,allocate,ledger,summarize,goalProgress};
