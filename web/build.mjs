...
app=app.replace("async function doAction(action,el){", "async function doAction(action,el){if(action.startsWith('range-'))return window.norte.rangeAction(action,el,{state,period,render});");
app=app.replace("period=`${year}-${String(month).padStart(2,'0')}`;", "window.norte.clearRange();period=`${year}-${String(month).padStart(2,'0')}`;");
app=app.replace('window.norte.refresh=refresh;', "for(const name of ['dashboard','annual','entries','budget']){const original=views[name];views[name]=()=>window.norte.getRange()?window.norte.rangeReport(state,name==='entries'?'debit':'all'):original();}window.norte.refresh=refresh;");
