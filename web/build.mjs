...
app=app.replace("async function doAction(action,el){", "async function doAction(action,el){if(action.startsWith('web-')){await window.norte.action(action,el);if(!['web-accounts','web-logout','web-profile','web-catalog'].includes(action))await refresh();return;}");
...
app=app.replace("${button(icon('sun'),'theme'", "${window.norte.rangeControl()}${button(icon('sun'),'theme'");
...
app=app.replace('window.norte.refresh=refresh;', "for(const name of ['dashboard','annual','entries','budget']){const original=views[name];views[name]=()=>name==='dashboard'||window.norte.getRange()?window.norte.rangeReport(state,name==='entries'?'debit':'all',period):original();}window.norte.refresh=refresh;");
