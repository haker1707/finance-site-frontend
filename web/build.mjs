...
app=app.replace('<div class="avatar" title="Conta local">EU</div>',"${window.norte.accountButton(state)}${button('Sair','web-logout','','small flat')}");
app=app.replace("async function doAction(action,el){", "async function doAction(action,el){if(action.startsWith('web-')){await window.norte.action(action,el);if(!['web-accounts','web-logout','web-profile'].includes(action))await refresh();return;}");
