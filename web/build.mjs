...
app=app.replace('<div class="avatar" title="Conta local">EU</div>',"${button('Contas','web-accounts','','small flat')}${button('Sair','web-logout','','small flat')}");
app=app.replace("async function doAction(action,el){", "async function doAction(action,el){if(action.startsWith('web-')){await window.norte.action(action,el);if(!['web-accounts','web-logout'].includes(action))await refresh();return;}");
...
app=app.replace("refresh().catch(e=>", "const desktopRender=render;render=function(){desktopRender();window.norte.onRender?.();};window.norte.clearPrivateData=()=>{state={records:[],settings:{}};route='dashboard';chosenFile='';importPlan=null;document.querySelectorAll('dialog').forEach(d=>{d.close();d.innerHTML='';});};window.norte.refresh=refresh;refresh().then(()=>window.norte.afterLoad()).catch(e=>");
