...
app=app.replace('Seus dados, no seu computador.<br>Modo offline · v1.0.0','Suas finanças, com privacidade.<br>Norte · Conta pessoal');
app=app.replace('<div class="avatar" title="Conta local">EU</div>',"<time data-live-clock class=\"live-clock\" aria-live=\"off\" aria-label=\"Data e hora do dispositivo\"></time>${window.norte.accountButton(state)}${button('Sair','web-logout','','small flat')}");
app=app.replace("async function doAction(action,el){", "async function doAction(action,el){if(action.startsWith('web-')){await window.norte.action(action,el);if(!['web-pro','web-accounts','web-logout','web-profile','web-catalog'].includes(action))await refresh();return;}");
...
app=app.replace("routes.splice(routes.length-1", "const debitView=views.entries;views.entries=()=>window.norte.financeTabs('entries')+window.norte.savedDebitView(state,period,query)+window.norte.recurringView(state,period);views.cards=()=>window.norte.cardsView(state,period,{entryTable});const overview=views.dashboard;views.dashboard=()=>overview()+window.norte.forecastView(state,period);routes.splice(routes.length-1");
...
// Delete directly from the editor; cancelling keeps the unsaved form intact.
app=app.replace("<div id=\"entity-error\"></div><div class=\"dialog-footer\">","<div id=\"entity-error\"></div><div class=\"dialog-footer\">${r.id&&!r.locked&&state.role==='owner'?'<button type=\"button\" id=\"editor-delete\" class=\"danger editor-delete\">Excluir</button>':''}");
app=app.replace("const form=$('#entity-form');", "const form=$('#entity-form');const deleteButton=form.querySelector('#editor-delete');if(deleteButton)deleteButton.onclick=async()=>{if(deleteButton.disabled||form.querySelector('[type=submit]')?.disabled)return;if(!confirm(kind==='purchase'?'Excluir esta compra e suas parcelas cadastradas?':'Excluir este registro salvo? Esta ação não pode ser desfeita pelo site.'))return;const controls=[...form.querySelectorAll('button,input,select')].map(x=>[x,x.disabled]);controls.forEach(([x])=>x.disabled=true);try{await api('remove',{id:r.id});$('#modal').close();await refresh();toast('Registro excluído.');}catch(error){if(form.isConnected)form.querySelector('#entity-error').textContent=error.message;}finally{controls.forEach(([x,disabled])=>{if(x.isConnected)x.disabled=disabled;});}};");
app=app.replace("case 'schedule':{const p=byId(id);","case 'schedule':{if(window.norte.openPurchaseSchedule)return window.norte.openPurchaseSchedule(id);const p=byId(id);");
app=app.replace("${button(icon('sun'),'theme'", "${window.norte.rangeControl()}${state.account?.isConsultant?button('Gestor','web-pro','','small'):''}${button(icon('sun'),'theme'");
app=app.replace("async function doAction(action,el){", "async function doAction(action,el){if(action.startsWith('range-'))return window.norte.rangeAction(action,el,{state,period,render,setPeriod:value=>{period=value;page=0;}});");
app=app.replace("period=`${year}-${String(month).padStart(2,'0')}`;", "window.norte.clearRange();period=`${year}-${String(month).padStart(2,'0')}`;");
app=app.replace('window.norte.refresh=refresh;', "for(const name of ['dashboard','annual','entries','budget']){const original=views[name];views[name]=()=>name==='dashboard'||window.norte.getRange()?window.norte.rangeReport(state,name==='entries'?'debit':'all',period)+(name==='dashboard'?window.norte.professionalClientPanel(state,period,window.norte.getRange()):''):original();}window.norte.refresh=refresh;");
app=app.replace(/<select id="month"[\s\S]*?<input id="year"[^>]*>/,'');
