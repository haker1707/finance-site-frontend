import {openProfessional,clearProfessional,professionalClientPanel} from './professional-ui.js';
import {rangeControl,rangeAction,rangeReport,getRange,clearRange} from './date-range.js';
import {openPurchaseSchedule} from './payments-ui.js';
import {mountClock,stopClock} from './clock.js';
...
function clearAccount(){clearProfessional();clearRange();stopClock();clearFinancial();accessEpoch++;lastState=null;workspaceId='';revision=0;selectedFile=null;preview=null;window.norte.clearPrivateData?.();const toast=document.querySelector('#toast');if(toast){toast.textContent='';toast.style.display='none';}}
...
async function call(action,p={}){
 ...
 if(action==='backup'){const state=await call('state');return download('Norte-backup-'+new Date().toISOString().slice(0,10)+'.json',JSON.stringify({format:'norte-web-1',exportedAt:new Date().toISOString(),data:{records:state.records,settings:state.settings,imports:state.imports,recurringRules:state.recurringRules||[],categoryRules:state.categoryRules||[],installmentPlans:state.installmentPlans||[],installmentIgnored:state.installmentIgnored||[],natureRules:state.natureRules||[],invoiceStatements:state.invoiceStatements||[],invoicePayments:state.invoicePayments||[],purchaseHistory:state.purchaseHistory||[],invoiceAutoIgnored:state.invoiceAutoIgnored||[]}},null,2));}
 if(action==='restore'){
...
async function action(action,el){
 if(action==='web-catalog')return openCatalog(request);
 ...
 if(action==='web-pro')return showProfessional();
...
window.norte={web:true,professionalClientPanel,rangeControl,rangeAction,rangeReport,getRange,clearRange,paymentState:()=>lastState,openPurchaseSchedule:id=>openPurchaseSchedule(lastState,id,{call,refresh:()=>window.norte.refresh(),getState:()=>lastState}),savedDebitView,entryMeta,entryActions,rulesView,offerCategoryRule:entry=>categoryDialog(lastState,entry,{call,refresh:()=>window.norte.refresh(),message}),call,action,cardsView,financeTabs,recurringView,forecastView,financialAction:(action,el,ctx)=>financialAction(action,el,ctx,{call,refresh:()=>window.norte.refresh(),message}),bankView,accountButton,settingsView:state=>state.role==='consultant'?'<section class="panel"><h1>Acesso do consultor</h1><p>Você pode consultar esta conta. Somente o responsável pode alterar registros, gerenciar acessos e conectar bancos.</p><button data-action="web-accounts">Selecionar conta</button></section>':settingsView(state)+'<section class="panel"><h2>Catálogo de categorias</h2><p>Consulte as regras genéricas e sugira melhorias sem compartilhar seu extrato.</p><button data-action="web-catalog">Abrir catálogo</button></section><section class="panel"><h2>Quem pode acessar</h2><p>Somente você e os consultores que autorizar. Os consultores têm acesso de leitura e não podem convidar outras pessoas.</p><button data-action="web-access">Profissionais autorizados</button></section>',onRender:()=>{
  mountClock();
  const readonly=lastState?.role==='consultant';
  if(readonly){const content=document.querySelector('#content');if(content)content.insertAdjacentHTML('afterbegin',`<div class="pro-client-banner"><div><strong>Acompanhando: ${escape(lastState.workspaceName||'Cliente autorizado')}</strong><p class="tiny">Acesso profissional de leitura · conta do cliente</p></div><button data-action="web-pro">Voltar aos meus clientes</button></div>`);}document.body.classList.toggle('consultant',readonly);
  if(!readonly)return;
  const allowed=new Set(['range-open','range-clear','range-page','navigate','collapse','csv','prev-page','next-page','schedule','web-pro','web-logout','web-accounts','web-profile','theme','web-reload','web-bank-filter','web-bank-prev','web-bank-next','finance-card','finance-back','finance-card-tab','finance-invoice-payment','finance-plan-schedule','finance-notifications','finance-part-open','finance-month','finance-calendar-year','finance-saved-month','finance-saved-filter','finance-saved-page']);
...
async function accessDialog(){
 ...
 dialog.innerHTML=`<div class="dialog-heading"><h2>Profissionais autorizados</h2><button type="button" id="close-access">Fechar</button></div><p>Cadastre o e-mail do profissional. Ele deve entrar no site com esse mesmo e-mail, confirmado, e aceitar o acesso. O Norte não envia convites por e-mail.</p><form id="grant-access"><div class="field"><label for="consultant-email">E-mail do consultor</label><input id="consultant-email" name="email" type="email" autocomplete="off" required></div><button class="primary" type="submit">Autorizar leitura</button></form><div id="access-error" role="alert"></div>${rows.map(row=>`<div class="bank-item"><strong>${escape(row.consultant_email)}</strong><p>${row.accepted_at?'Acesso aceito':'Aguardando aceite'}</p><button type="button" data-revoke="${escape(row.id)}">Revogar acesso</button></div>`).join('')||'<p>Nenhum profissional autorizado.</p>'}<p class="tiny">A revogação bloqueia as próximas consultas. Informações já visualizadas ou exportadas pelo consultor não podem ser recolhidas.</p>`;
...
async function showProfessional(){
 const account=await request('account-get');if(!account.isConsultant)throw Error('Ative seu perfil profissional com um código em Minha conta → Perfil profissional.');
 clearAccount();document.body.classList.remove('consultant');
 await openProfessional({request,personal:async()=>{try{clearAccount();const available=await request('workspaces');workspaceId=available.own.id;await loadApp();}catch(e){message(e.message,true);}},client:async id=>{try{clearAccount();workspaceId=id;await loadApp();}catch(e){message(e.message,true);}},logout:()=>accountLogout('Você saiu do Norte.')});
}
...
setInterval(()=>{if(loaded&&workspaceId&&document.visibilityState==='visible')request('access-check').catch(error=>{if(lastState?.role==='consultant'){clearAccount();document.querySelector('#app').innerHTML='<section class="auth-card"><h2>Acesso precisa ser confirmado</h2><p>Não foi possível confirmar a autorização. Reabra o Norte para continuar.</p><a href="./">Reabrir</a></section>';}message(error.message,true);});},15000);
