...
window.norte={web:true,savedDebitView,entryMeta,entryActions,rulesView,offerCategoryRule:entry=>categoryDialog(lastState,entry,{call,refresh:()=>window.norte.refresh(),message}),call,action,cardsView,financeTabs,recurringView,forecastView,financialAction:(action,el,ctx)=>financialAction(action,el,ctx,{call,refresh:()=>window.norte.refresh(),message}),bankView,accountButton,settingsView:state=>state.role==='consultant'?'<section class="panel"><h1>Acesso do consultor</h1><p>Você pode consultar esta conta. Somente o responsável pode alterar registros, gerenciar acessos e conectar bancos.</p><button data-action="web-accounts">Selecionar conta</button></section>':settingsView(state)+'<section class="panel"><h2>Catálogo de categorias</h2><p>Consulte as regras genéricas e sugira melhorias sem compartilhar seu extrato.</p><button data-action="web-catalog">Abrir catálogo</button></section><section class="panel"><h2>Quem pode acessar</h2><p>Somente você e os consultores que autorizar. Os consultores têm acesso de leitura e não podem convidar outras pessoas.</p><button data-action="web-access">Gerenciar consultores</button></section>',onRender:()=>{
  ...
  if(!readonly)return;
  const allowed=new Set(['navigate','collapse','csv','prev-page','next-page','schedule','web-logout','web-accounts','web-profile','theme','web-reload','web-bank-filter','web-bank-prev','web-bank-next','finance-card','finance-back','finance-card-tab','finance-notifications','finance-part-open','finance-month','finance-calendar-year','finance-saved-month','finance-saved-filter','finance-saved-page']);
