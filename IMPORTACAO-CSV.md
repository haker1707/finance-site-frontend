...
## Catálogo compartilhado, notificações e relógio — setembro de 2026

- Catálogo genérico versionado no Supabase: Uber/Trip → Transporte, Uber One/Pass → Assinaturas, Mercado Livre → Compras online e Mercado Pago somente como intermediador. Expressões são normalizadas, com limites de palavra, exclusões, prioridade e desempate estável. Não há upload automático de descrições, valores ou regras pessoais para o catálogo.
- A escolha na conferência prevalece sobre regra pessoal, catálogo e padrão Outros. Removidas as antigas inferências genéricas fixas e a reutilização implícita de categorias pelo histórico. Padrões mensais continuam sendo sugestões de possível assinatura, nunca comprovação de assinatura; parcelas são excluídas dessa detecção.
- Prévia e servidor usam os mesmos módulos puros. Uma importação aberta guarda as versões do catálogo consultado; o servidor recupera essas versões aprovadas, mantendo as escolhas. Se indisponível, a interface informa a limitação e mantém regras pessoais/Outros. Não há recategorização retroativa.
- Configurações → Catálogo permite consultar regras e enviar sugestões genéricas explicitamente. Até 20 sugestões pendentes por usuário; mostra até 100 sugestões recentes. Administradores podem criar, editar, desativar, aprovar e rejeitar. O catálogo admite até 500 regras lógicas, preservando suas versões.
- Consultas autenticadas ao catálogo; sugestões privadas ao autor por RLS. Escritas somente via servidor, que verifica autenticação e autorização administrativa atual, e publica versões/aprovações atomicamente. Nenhuma chave privilegiada no frontend.
- Cartão: cabeçalho com sino e ações, abas, calendário, resumo selecionado e lista compacta. Sino derivado dos estados existentes de conferência e vínculo ambíguo, sem novos registros de pendência. Erros e duplicidades de CSV bloqueados antes de gravar não são apresentados como novas pendências salvas. Não inferimos problemas em dados antigos.
- O sino abre um diálogo acessível e leva ao mês, filtro e página do lançamento, preservando seleções. Vínculos ambíguos abrem sua resolução para o titular. Totais previstos permanecem indisponíveis enquanto há ambiguidades no cartão.
- Relógio no cabeçalho, atualizado localmente a cada segundo no fuso do dispositivo, independente do período financeiro e sem chamadas de rede. Temporizador removido quando o componente sai da página. Leitores de tela não recebem anúncios a cada segundo.
- Revisão de código/configuração, análise de segurança do Supabase, sintaxe, compilação e publicação são distintas de validação em uso real. Nenhum teste automatizado/manual, arquivo financeiro importado ou movimentação fictícia criado nesta entrega.

## Natureza financeira e duplicidades — protocolo CSV 3

- `import-rules.mjs` é compartilhado pelo navegador e servidor e separa natureza de categoria. Faturas Nubank usam negativo como crédito; extratos Nubank usam positivo como entrada. Formatos mapeados oferecem ambas as convenções e a opção de classificação manual.
- Pagamentos/adiantamentos explícitos e estornos/reembolsos/devoluções explícitos são identificados conforme sinal e contexto. “Estorno de pagamento”, sinais conflitantes e créditos sem descrição conclusiva exigem confirmação específica. IOF negativo pode sugerir estorno, sem presumir vínculo com a compra. Histórico compatível é apenas evidência; não cria vínculo ou quitação.
- Conferência humana permanece separada da identificação automática. A lista oferece decisões rápidas e a edição completa. Natureza confirmada fica vinculada por SHA-256 aos dados daquela decisão; editar descrição, data, valor ou natureza invalida confirmações afetadas. Conferência posterior não dispensa essas validações.
- Regras pessoais de natureza são opcionais, limitadas a banco/formato, cartão ou conta, convenção e expressão específica. São guardadas no documento privado do titular e gerenciadas em Categorias → Regras de natureza. Não são enviadas ao catálogo compartilhado. Criar/editar/remover usa autenticação, autorização de titular e a mesma gravação atômica com revisão do documento.
- Identidade CSV v3 registra hash do arquivo e linha original; identificador bancário, quando confiável, é limitado ao banco e à conta/cartão. Em formato mapeado, o usuário precisa declarar que a coluna contém identificadores únicos de movimentação. O `csvKey` antigo é preservado, sem regravar dados existentes.
- Identidades históricas sem identificador suficiente passam por comparação e confirmação, em vez de presumir repetição apenas por data/descrição/valor. Um arquivo histórico comprovadamente já importado, com destino e identidade compatíveis, mantém bloqueio. Novas identidades continuam protegidas após edição do lançamento salvo.
- Possíveis duplicidades consideram conta/cartão, banco quando conhecido, natureza, sinais/convenção, valor, descrição normalizada, data/mês e número/total de parcelas. Débito nunca compara contas diferentes. Cobrança e estorno não se confundem. A repetição legítima sem identificador único permanece uma decisão do titular.
- Prévia e servidor comparam o lote completo antes de salvar. A comparação sobreposta mostra até 20 candidatos por lançamento, conta todos e considera todos nas assinaturas de confirmação. Grupos de evidência são resumidos por SHA-256 para limitar memória e tamanho da requisição. Alterações em candidatos invalidam a confirmação; a ordenação dos arquivos não escolhe quais semelhanças são mostradas como possíveis.
- Reimportações comprovadas dentro do lote elegem uma única origem por ordenação estável de arquivo/linha; cópias restantes ficam bloqueadas. Possíveis duplicados começam desmarcados. “É duplicado — não importar” nunca exclui o registro salvo. Linhas omitidas são explicitamente contadas na confirmação e registradas no relatório de importação.
- O protocolo envia decisões inclusive das linhas desmarcadas e o servidor revalida o mesmo conjunto. Clientes antigos precisam reabrir a importação após atualizar o site; não há conversão silenciosa de decisões antigas.
- Não houve mudança de schema: regras e metadados versionados utilizam o JSON privado existente em `norte_documents`, protegido pelas permissões/RLS existentes e por `norte_commit`. Portanto não foi necessária migração SQL. Nenhum dado financeiro existente foi transformado.
- Limitações: sem identidade confiável, o sistema apresenta suspeitas e não comprova que compras semelhantes sejam o mesmo evento. Não concilia automaticamente pagamentos entre débito e crédito, não marca faturas pagas e não vincula estornos a compras de forma definitiva. Somente revisão de código/configuração e compilação; nenhum teste automatizado ou manual ou importação de arquivos do usuário.
